...
export const sendSignUpEmail = inngest.createFunction(
  // Creating a Inngest (background) function called sendSignUpEmail
  ...
  async ({ event, step }) => {
    ...
    const prompt = PERSONALIZED_WELCOME_EMAIL_PROMPT.replace(
      // taking the predefined prompt and personalizing it with the user's info
      "{{userProfile}}",
      userProfile,
    );

    const response = await step.ai.infer("generate-welcome-intro", {
      // step.ai.infer() is Inngest's AI step (giving it the id - generate-welcome-intro) - tells ingest to generate the prompt and save it in a variable called response
      model: step.ai.models.anthropic({
        model: "claude-haiku-4-5",
        defaultParameters: { max_tokens: 300 },
      }), // Specify the model
      body: {
        max_tokens: 300,
        messages: [
    ...
    await step.run("send-welcome-email", async () => {
      // This creates another inngest step (with id send-welcome-email)  - this step is to actually send the email
      ...
      return await sendWelcomeEmail({
        // calls sendWelcomeEmail to actually send the Email, with the user data
        email: data.email,
        name: data.name,
        intro: introText,
      });
